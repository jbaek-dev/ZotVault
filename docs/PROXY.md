...
ZotVault fetches PDFs open-access-first (arXiv, Unpaywall). For papers that
...
1. You log in to your library proxy **in your normal browser** (Duo/2FA and
   all). ZotVault never touches your password.
2. You export that browser session's cookies as a Netscape `cookies.txt`.
3. ZotVault rewrites the paper's DOI URL through the proxy template, rides
...
When the session expires, downloads stop working and `zotvault trace` shows
...
private, e.g. `~/.zotvault/proxy_cookies.txt` (readable only by you:
`chmod 600`).
...
cookie_file = "~/.zotvault/proxy_cookies.txt"
daily_limit = 10
request_delay_sec = 10
```

Run `zotvault doctor` to validate, then `zotvault run-once` — items whose
...
publishers automatically block campuses when they detect it. ZotVault's
...
all: leave `[proxy] enabled = false` and ZotVault's normal downloader will
